...
| 接口 | 从 CF 边缘单发成功率 | 失败形态 | 本机直连 |
| ---- | -------------------- | -------- | -------- |
| `push2his`（沪深300 基准线） | **~4%**（7 个批次 6 败 1 成） | 快速连接重置（失败轮只多 1.15s） | >98% |
| `lsjz`（基金净值历史） | 未单独统计 | **我们自己的 8s 超时**（`AbortError`） | 正常 |

结论：**重试救不了 push2his**——4 次重试只把一轮成功率从 4% 抬到 ~14%，要 95%
得 ~75 次尝试，而代价全落在访客 TTFB 上（实测失败轮 +1.15s、成功轮最高 +7.3s）。
`fetchIndexNav` 的 stale 兜底是必需品，不是保险。彻底解决只有两条路：换源，
或本地/CI 抓一次 `wrangler kv key put` 进 KV、页面只读（**尚未做，需主人定夺**）。
...
2. **日志分层**：有兜底 → `warn`（数据在变旧，页面照常）；无兜底 → `error`
   （用户真的看不到）。并且 `console.error(msg, err)` 会被 CF 日志管线**丢掉
   err 的 message、只留 stack**（同一份日志里 `AbortError: ...` 保留、原生网络
   错误只剩帧），所以关键原因要 `String(err)` 拼进模板。
3. **别让访客替全网抽奖**：长历史回填有闸门 `fund.nav_backfilled_at`
   （`domain/nav-backfill`，不论成败记 6 小时 ⇒ 单基金 ≤4 次/天）。光看
   「行数 < 60」对**上市不足 60 个交易日的新基金**永久成立——实测 028439
   上游总共只有 50 行，于是每次访问白拉 3 页东财 + 写一遍 D1，且数据停在
   12 天前（它不在 cron 同步清单里：无持仓、无待确认单）。
...
- **强制让某条 KV 缓存失效**去观察降级路径：删主 key、往 `:stale` 兜底 key
  写一个「只有 10 天」的哨兵，然后打页面——页面里基准点数是 9 还是 260 就
  直接暴露「上游到底有没有被真的打、成没成」。
