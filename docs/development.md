...
**消费方有三处**（第三处 2026-09-23 起）：
1. 撮合：`resolveConfirmDate` 算 T+1 确认日；
2. 定投：`nextRunDate` 算下次执行日（周末/节假日不下单）；
3. 基准线取数窗口：`lastClosedTradingDay` 算「最后已收盘的交易日」，做
   `fetchIndexNav` 的窗口右端（口径：≥15:00 且当天是交易日 → 当天，
   否则回退到上一个交易日；与撮合「15:00 整算之后」刻意不同，理由见函数注释）。

表漏了某天，定投会多生成一单（会在下个交易日被撮合），基准线窗口也可能多算
或少算一天。兜底机制：撮合时会把 `fund_nav` 里的净值日期序列作为
`knownTradingDays` 传入——有净值的那天必然是交易日，可反向校正节假日表的遗漏。
...
## ⚠️ 边缘拉东财：降级路径的纪律（2026-09-23）

### 实测数字（线上探针 + 日志，别凭感觉改策略）

| 接口 | 从 CF 边缘 | 失败形态 | 本机直连 |
| ---- | ---------- | -------- | -------- |
| `push2his`（沪深300 基准线） | 单次 HTTP 成功 **~4%**（一轮 4 次重试的整体成功 ~14%） | 快速连接重置（失败轮只多 1.15s） | >98% |
| `lsjz`（基金净值历史） | 单页 **7~8s**（能到，但贴着超时线） | 我们自己的超时（`AbortError`） | ~0.3s |

> 口径别混：**~4% 是「单次 HTTP 尝试」**的成功率（由 7 个批次里 6 败 1 成反推，
> `(1-p)^4 = 6/7`）；**~14% 是「一轮 4 次重试」**的整体成功率。两个量纲。

结论：**重试救不了 push2his**——要把一轮抬到 95% 得 ~75 次尝试，代价全落在访客
TTFB 上（实测失败轮 +1.15s、成功轮最高 +7.3s）。`fetchIndexNav` 的 stale 兜底是
必需品，不是保险。彻底解决只有两条路：换源（要求「从边缘可达」，**尚未定源**），
或本地/CI 抓一次写 KV、页面只读。

**已知取舍（刻意接受，别再当 bug 修）**：基准线拉不到时页面照画 stale 数据，
**UI 上没有任何「这是旧数据」的提示**，且 stale 只在上游成功时才刷新 ⇒ 它的
年龄**没有上界**，可能与基金线末端错位很久。接受它的理由是：这是一条装饰性
对比线，宁可给旧的也别给空白；而「让用户看清基准线是旧的」需要 UI 改造，
收益不抵成本。真要根治就走上一条（换源/离线预抓）。

### 六条写代码时的纪律

1. **HTTP 200 但 `data` 为空 = 失败**，必须和抛异常走同一条兜底
   （2026-09-23 修：原先这条出口静默返回空数组，基准线整段消失且无日志）。
2. **日志分层**：有兜底 → `warn`（数据在变旧，页面照常）；无兜底 → `error`
   （用户真的看不到）。原因**自己拼进模板**：同一批线上日志里，`DOMException`
   （`AbortError: The operation was aborted`）保留了 message，而原生网络错误
   只剩 stack ⇒ 只靠 `console.error(msg, err)` 会看不出来是谁的错。
   注意这条纪律的**代价**：失败窗口里每次访问都会产出一条 warn（上游成功率
   只有 ~4% 时这很频繁），暂未做节流/负缓存——要盯日志量。
3. **别让访客替全网抽奖**：长历史回填有闸门 `fund.nav_backfilled_at` +
   `nav_backfilled_target`（`domain/nav-backfill`）。光看「行数 < 60」对
   **上市不足 60 个交易日的新基金**永久成立——实测 028439 上游总共只有 50 行
   （D1 里当时 43 行，差的 7 天是后面补上的两个不同口径），于是每次访问白拉
   3 页东财 + 写一遍 D1（它不在 cron 同步清单里：无持仓、无待确认单）。
   闸门口径：**成功**记满 6 小时；**失败**只记 30 分钟冷却（拉空不写任何数据，
   若也锁 6 小时，上游抖一下就成了「数据停一天」）；**目标更大时放行**
   （回测页要 250 行，不该被基金页那次 60 行连坐 6 小时）。
4. **时间预算分档**（2026-09-23）：有人等着的路径用 `NAV_FETCH_TIMEOUT_MS`（8s），
   cron / 后台回填用 `NAV_FETCH_TIMEOUT_BACKGROUND_MS`（12s）——实测 lsjz 单页
   从边缘要 7~8s，正贴着 8s 线，一页被掐断就是「今晚订单整晚顺延」。另外
   lsjz 翻页**某一波全败即收手**，别把剩下波次一路打完（400 行 = 4 波）；
   `syncNav` 还有一层 **8 分钟总预算**（cron 墙钟硬顶 15 分钟，超预算收手，
   剩下的留给 21:30 那轮幂等重跑）。
5. **别让访客等回填**：基金页 loader 只在「库里一行都没有」时才 `await`
   回填（没数据的页是废页，爬虫只来一次）；有数据但残缺时**现有序列先渲染**，
   回填丢 `ctx.waitUntil`——下次访问或爬虫就齐了。
6. ⚠️ **`ctx.waitUntil` 的预算是 30 秒**（官方 limits：响应发出后最多 30s，
   **同请求内所有 waitUntil 共享**，超时未结算的 Promise **被取消并记警告**），
   而 Free 档 **CPU 只有 10ms/调用**，后台任务与 SSR 共享。所以后台任务只能用
   **一次翻页波**的量级（`NAV_PAGE_BACKFILL_MAX_ROWS = 100`）：400 行 = 20 页
   = 5 个串行阶段 ≈ 35~60s，会被平台掐死**却什么都没写**（闸门还会白烧）。
   长历史回填因此**不在请求里做**，留给回测页（用户主动等）与后续 cron/队列。

### 排查手法备忘

- **强制让某条 KV 缓存失效**去观察降级路径：删主 key、往 `:stale` 兜底 key
  写一个「只有 10 天」的哨兵（计数口径：首行内联了 `"date"/"close"` 两个 key，
  所以页面里数出来的基准对数是 **9**），然后打页面——是 9 还是 260 就直接暴露
  「上游到底有没有被真的打、成没成」。
- ⚠️ **KV 有 60 秒边缘读缓存**：删 key 后 60 秒内应用可能仍读到旧值，两次探针
  之间要留 ≥60s，否则把「缓存命中」误读成「没打上游」。
- ⚠️ **本机 `wrangler tail` 连不上**（`connect ETIMEDOUT 103.252.115.169:443`，
  与 GitHub 同类的网络限制），实时日志要看 CF 控制台的 Observability 面板。
  另外 `wrangler tail` 命令退出时**不会杀掉它后台起的子进程**——别把关键请求
  放在它后面的 `&` 子 shell 里，否则你以为没跑、它其实跑了（2026-09-23 实测）。
