...
| --- | --- | --- |
| `fund:basic:{code}` | 3 天 | 1/3 |
| `fund:detail:{code}` / `fund:position:v2:{code}` / `fund:alloc:{code}` / `fund:bonus:{code}` / `fund:manager:{code}` / `fund:style:{code}` | 7 天 | 各 1/7 |
| 合计 | | **≈ 1.19** |

⚠️ **1.19 是「被反复访问」的稳态均值，不是单日上限**：TTL 从写入时刻起算、且只在未
命中时才写，所以「首次全量抓取」或「6 个长 TTL key 齐过期后重抓」的那一天仍是
113 × 7 = **791 次（79%）**，与调 TTL 之前一模一样，只是从「每天如此」变成「每 7 天
一次」。按 7 个 1 天档算，1000 ÷ 7 ≈ **143 只/天**写满；按现在的 1.19 算，约 **840
只/天**才会写满。要压低**单日峰值**只能靠基金页的边缘缓存（已就位，见
`docs/china-access.md`）或把这类纯东财镜像缓存挪出 KV——继续拉 TTL 做不到。

现已按**数据自身的变动频率**分档：档案（费率/起购/申赎状态）3 天；搜索词与
排行榜留 1 天（前者 key 空间无界、后者用户直接看数字）；季度级披露与只增不改的
（重仓股/资产配置/分红/经理/风格/详情）放 7 天。
...
KV 自身不可用时（读额度用尽 / API 抖动 / 5xx）不影响页面：读走 `safeGet`（当未命中，
继续回源）、写走 `safePut`（只记一条日志）。缓存是旁路，它挂了只该让页面变慢，
不该让页面变空或 500。
...
再不够用时按顺序考虑：继续拉长 `basic`（**必须同步 `ensureFund` 的窗口**——源码里是
`FUND_PROFILE_FRESH_MS = CACHE_TTL.basic * 1000`，两者本就绑在一根绳上，守卫测试
「ensureFund 档案新鲜度窗口」钉住了它）→ 这类「纯东财镜像缓存」换成 Cache API
