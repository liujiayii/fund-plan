...
完整施工步骤。**最终状态：不绑卡，入口维持老域名**（新域名试签未中奖、
SaaS 因需绑支付方式搁置，见「实测结论」第三轮）；日后若绑卡，按 B1
步骤可再砍一半延迟。
...
### 第三轮 16:50（新域名试签）：未中奖，B1 搁置

新域名 `liujiayi.dpdns.org`（IP 对 104.21.47.129/172.67.171.38）全国实测：
电信 **AMS×40（73%）** 依旧绕欧洲，联通/移动的机房分布与老域名统计等价
（本轮 TTFB 普遍更高是晚高峰拥塞，非域名因素）。最终决定：
...
- 继续摇号（注册更多免费域名抽 IP 对）期望值低：两个样本的电信分布
  都是 AMS 主导，免费 zone 抽到的 104.21.x/172.67.x 段就是这个命

## B1 · Cloudflare for SaaS 优选 IP（搁置：需绑卡；步骤保留备用）
