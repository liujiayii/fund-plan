...
### 5.3 定投回测工具页（`/tools/dca-backtest`）

- **为什么单独一页**：基金页那张回测卡的口径是写死的（每月 1000 元 × 最近 12 期），
  而「定投收益 / 定投回测」这类词的用户意图是**自己想试**。这一页把口径做成可调：
  每期金额、**频率**（按月 / 按周 / 按天）、**期数**（可自定义，按频率默认 12 / 52 / 250）、
  净值口径（累计净值 / 单位净值）。
- **频率的「期」怎么定义**（改实现要同步页面文案）：按月 = 每个自然月首个有净值的交易日；
  按周 = 每个**自然周**（周一~周日）的首个交易日——**不是「每 7 天」**，否则跨年那一周
  会被切成两期（2025-12-29 与 2026-01-02 本是同一周）；按天 = 每个交易日都买。
  期数 = 「最近 N 期」，所以期数与频率一起决定窗口长度。可用期数由库里净值长度决定
  （回填的是 400 行**交易日**净值 → 按天最多约 400 期、按周约 86、按月约 19；
  2026-09-17 在本地实测：日频提示「最多 400 期」、周频「最多 86 期」），
  填超了按可用期数算并提示。
- **计算在服务端**：URL 即状态（`?code=&amount=&freq=&periods=&adjust=`），表单 GET 提交让
  loader 重算——首屏的数字、逐期明细、区间全在 SSR 的 HTML 里，爬虫不跑 JS 也读得到，
  分享出去的链接带同一套参数。canonical 剥 query，参数变体不会变成一堆重复 URL。
  明细表按天可能有几百行，所以开了分页：SSR 只渲染当前页，单页时不显示翻页器。
- **不带参数的版本必须有真数字**：进 sitemap 的就是 `/tools/dca-backtest`（无 query），
  所以没带 `code` 时取库里**最近更新过**的那只基金当默认标的。否则爬虫抓到一张空表，
  而这一页存在的意义就是「值得被收录的独有内容」。真正的空白兜底只留给
  「库里一只基金都没落档」的冷启动。
- **口径与基金页共用一份实现**（`app/domain/dca-backtest.ts`）：每月首个交易日、内扣申购费、
  累计净值、最大回撤取已投份额市值的峰谷。这一版**新增两条**：年化走资金加权 **XIRR**
  （日粒度现金流二分求解），且**区间不足半年返回 null**——3 期 88 天的 +50% 折出来是
  +1079%/年，数学没错但会误导人；页面写「区间不足半年，不折算」。
- **参数口径单独成模块**（`app/domain/dca-backtest-params.ts` + 单测）：默认值、边界、
  回落文案都在那里。URL 是用户与爬虫都能直接改的输入，非法值一律**回落默认值 + 留一条提示**
  （用户填了 50 元却看到按 1000 元算的数字，比报错更糟）。
- **入口**：`SITEMAP_STATIC_PATHS` 加一条 + `NAV_ITEMS` 挂「定投回测」（§5.2 的约定）。
  ⚠️ 新增导航项**必须同时登记** `app/components/NavLinks.tsx` 的 `NAV_ICON`：那里是
  `NAV_ICON[key] ?? <UserOutlined />`，漏登记不报错，只是悄悄显示成「我的」的图标
  （2026-09-17 加定投回测时踩过）。`tests/domain/seo-guard.test.ts` 已加守卫：
  每个 `NAV_ITEMS` 的 key（含运行时追加的 `/admin`）都必须登记。
- 数据路径：`ensureNavHistory`（`app/services/portfolio-service.ts`）负责「净值少于 60 条就
  回填 400 天」。基金页 loader 里还有一份内联的同款逻辑（那一页当时未动），改口径时两处同改。
- 本地验证：

```bash
curl -s "http://localhost:5173/tools/dca-backtest?code=000001" | grep -o '逐期明细\|最大回撤\|口径说明'
curl -s "http://localhost:5173/tools/dca-backtest?code=000001&amount=2000&periods=6" | grep -o '每月\|累计投入'
curl -s "http://localhost:5173/sitemap.xml" | grep -c 'tools/dca-backtest'   # = 1
```

### 5.4 低估指数定投计划（`/plan`）

- **为什么单独一页**：`/master` 是「他持有什么」的全景账本，这一页是
  「他这周买了什么、为什么这么买、我怎么跟」的**一期复盘 + 换算工具**。
  塞进 `/master` 的 tabs 里两件事都讲不清（那边已经有四块）。
- **「本期」怎么定**（改实现必须同步页面文案）：「本期」= **不晚于今天、最近一个
  主理人有买入的周二**。他这周二还没买就**停在上一期**，页面顶部明说
  「主理人本周二还没买入，以下停留在 YYYY-MM-DD 这一期」。只认周二的买入——
  其它日子的机动加仓不纳入这个按周发布的计划。
- **数据全部来自真实委托**（`orders` 表，`app/services/undervalued-plan.ts`）：
  页面不维护一份写死的品种名单，主理人换了品种，页面跟着变。`pending` 也算
  「买了」（T+1 说的是份额到账，不是下单这个动作），`failed` / `cancelled` 不算。
  查询数是常数（订单 + 基金名两条），与期数、品种数无关。
- **一键换算**：**主理人本期买入合计 × 跟投比例**，再按各基金的买入金额占比拆到每只
  （`app/domain/undervalued-plan.ts` 的 `allocateByWeight`，**最大余数法**，
  保证 Σ 分配 == 目标总额、**一分不差**——用户把页面上九个数字加起来必须正好
  等于页面上那个合计）。URL 即状态（`?ratio=`，人话单位：百分比，可超过 100% 按倍数跟），
  非法值一律回落或收到边界并留提示。**没有「可用资金」输入框**：基准就是主理人的
  买入合计，用户不必再填一遍自己的钱（主人 2026-09-22 定稿的口径）。
- **简称自动缩**（`app/domain/fund-name.ts`）：库里的 `fund.name` 存的是东财
  SHORTNAME，本身就是「博时标普500ETF联接A」这种完整写法；这一页把它剥成
  「**标普500**」（去公司前缀 + 去 ETF联接/份额类别等载体后缀）。剥不动就原样退回，
  绝不显示残片。口味不对时往 `FUND_SHORT_NAME_OVERRIDES` 加一行按代码覆盖。
- **入口**：`SITEMAP_STATIC_PATHS` 加一条 + `NAV_ITEMS` 挂「低估定投计划」
  （位置紧跟「主理人的盘」）+ `NavLinks` 的 `NAV_ICON` 登记灯泡图标——
  三条缺一不可，§5.3 那条守卫钉着图标登记。
- **H1**：这一页的 hero 用真 `<h1>`（不是 antd `Title level={3}`）。全站目前只有
  首页有 h1（§2 表格里记为技术债），新页不该把这个坑再抄一遍；写法是裸 `<h1>`
  挂工具类定字号，不碰 antd 的 `level`/`component`，所以没有那条「改 h1 会连带
  字号漂移」的副作用。
- 本地验证：

```bash
curl -s "http://localhost:5173/plan" | grep -o '本期实盘定投品种\|一键算出本期金额\|指数基金投资理念'
curl -s "http://localhost:5173/plan?ratio=50" | grep -o '本期合计'   # 换算在服务端算
curl -s "http://localhost:5173/sitemap.xml" | grep -c 'liujiayii.dpdns.org/plan'  # = 1
```
