...
> 2026-08-25 · 状态：**四期均已施工**（2026-08-26 收尾）
> 前置文档：[`2026-08-24-fund-simulator-design.md`](./2026-08-24-fund-simulator-design.md)
>
> | 期 | 实现区间 | 状态 |
> | -- | -------- | ---- |
> | 期一 · 视觉地基 | `669ec88..64fa866` | 已完成（Task 11 收尾验收 `338aedc`） |
> | 期二 · 我的资产 | `801549f..2e4ecd1` | Task 1–5 已落地，⏳ **`## Self-Review` 尚未走过** |
> | 期三 · 交易体验 | `89075ce..25e3f4b` | 已完成（并线 squash `f7badbd`，PR #7） |
> | 期四 · 发现与详情 | `5f74dcf..2d59b74` | 已完成 |
>
> 各期逐 Task 明细见 `docs/superpowers/plans/` 下对应计划文档的状态戳。
>
> 本文只描述**前端体验与其所需的领域/服务能力**。撮合引擎、精度铁律、
> T+1 规则、权限模型一概不动 —— 那些在前置文档里已经定稿且有测试保护。
>
> ⚠️ **本文的「桌面为主」定位已于 2026-08-28 被
> [`2026-08-28-phase5-mobile-adaptation-design.md`](./2026-08-28-phase5-mobile-adaptation-design.md)
> 推翻。** 受影响的四处已就地标注（§1 定位边界、§2 设计原则 1、§12、§13）。
> 本文其余部分（配色体系、组件分层、资产重放、阶段涨幅、自选）**全部仍然有效**，
> 期一到期四的成果未因此返工。
...
| 维度 | 决定 |
| ---- | ---- |
| 形态 | ~~**桌面为主**，继续用 antd，不引入 antd-mobile、不做底部 Tab~~ ⚠️ **已被期五 spec 部分取代**：定位改为「双端平权」，底部 Tab **做**（手写 `<nav>` + CSS）。「不引入 antd-mobile」这条**仍然有效** |
...
1. **抄交互结构，不抄像素。** 支付宝真正好的是信息层级：一个巨大的总资产、
   ...
   不试图在桌面上复刻手机的尺寸与手势。
   > ⚠️ 期五 spec 缩小了本条的适用范围：**桌面端**仍不复刻手机尺寸与手势，
   > 但窄屏（<768px）现在有专门的布局与触控尺寸。本条不再等于「不做移动端」。
...
- ~~移动端 App 观感 / `antd-mobile` / 底部 Tab 栏 —— 已定桌面为主~~
  ⚠️ **已被期五 spec 部分取代**：底部 Tab 栏与移动端适配**移出排除项**。
  `antd-mobile`、原生手势 / 下拉刷新 / 转场动画**仍在排除项**（期五 spec §12）。
- 暗色模式
- 多只基金对比叠加、组合诊断、AI 荐基
- 消息中心 / 交易推送 / 邮件通知
...
| 决策 | 选择 | 理由 |
| ---- | ---- | ---- |
| 组件库 | 继续 antd | ~~定位桌面为主；~~换 antd-mobile 等于重写所有页面。⚠️ 期五改为双端平权后**本决策不变**：底部 Tab 手写 `<nav>` + CSS，零新增依赖 |
