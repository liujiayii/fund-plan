...
export const FP_CHART_THEME = {
  // 主序列默认色（无 colorField 的单线/面积）：冰川青蓝
  color: COLOR.primary,
  // 多序列色环：冰蓝 → 中性月灰 → 跌翡翠 → 待办金 → 冰白。第二位刻意给灰——
  // 基准线/债券等第二序列是辅助角色，与主线同属冰蓝区会缠成一片（用户走查实测）；
  // 第四位吃 pending 第三语义（宪法 §2.4），不再有图表专用字面量；
  // 冰白压到第五位做五序列兜底。NavChart 的基准线另有显式 color 覆盖
  category10: [
    COLOR.primary,
    COLOR.neutral,
    COLOR.down,
...
/**
 * 面积渐变填充（AssetTrendChart 用）：冰蓝 32% → 冰白 0%，自上而下淡出
