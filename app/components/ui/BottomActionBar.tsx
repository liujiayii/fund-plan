 ...
 * 实色底 + 朝上阴影已退役；窄屏同样贴底（2026-09-14 起任务页 tabbar 让位藏起，
 * 底部只剩这一层铬，不再叠在胶囊上方——两层玻璃零缝贴脸的时代过去了）。
 *
 ...
 * 既有底 padding（88px+safe-area，恰好与胶囊让位同值）负责。
 *
 * 定位属性（left/right/bottom/z-index）全部交给 responsive.css 的
 * .fp-bottom-bar（桌面 bottom:0，窄屏 bottom 让 iOS 安全区）——
 * inline style 斗不过类选择器，干脆不写，与 fp-tabbar 同手法。
 * 内层 max-width 容器挂 fp-bottom-bar-inner：窄屏内距收窄（24→16）由
 * responsive.css 覆盖，同样不跟 inline style 打架。
 */
export function BottomActionBar({ actions, note }: BottomActionBarProps) {
  return (
    ...
    >
      <div
        // 内层 max-width 容器：类名给 responsive.css 窄屏收窄内距用（24→16）
