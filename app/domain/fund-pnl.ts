 ...
 *   改任何一个函数都必须让那组测试保持全绿。
 *
 * ⚠️ 不变量的首日边界：dateAxis 首日若恰有确认订单，两侧不等——重放按首日
 *   约定 dayPnl ≡ 0（无前一日可比），归因却仍会给出非 0 条目（如买单的
 *   −申购费）。实际可达性极低：15:00 前下单确认日=当日，要让它落在首日，
 *   得同时凑齐「新基金上市首日 + 用户当天注册 + 15:00 前买入 + 当晚出首条
 *   净值」的组合（首日之前既无净值也无流水，确认订单才可能落在首日）；
 *   从次日起有前一日可比，不变量自愈。
 */
