...
import { getDcaPlans, type DcaPlanView } from '~/services/portfolio-service';
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  try {
    if (intent === 'create') {
      ...
      if (!/^\d{6}$/.test(fundCode)) return { error: '请输入 6 位基金代码' };
...
export default function MeDca({ loaderData }: Route.ComponentProps) {
  ...
  // 创建成功后关掉弹窗。必须放在 effect 里——渲染期调 setState 是反模式
  useEffect(() => {
    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card>
        ...
        <Paragraph type="secondary" style={{ marginTop: 12, marginBottom: 0 }}>
          系统每天北京时间 <Text strong>10:00</Text> 扫描到期的定投计划并自动下单，
